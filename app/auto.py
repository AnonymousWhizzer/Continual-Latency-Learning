...
def load_history_file():
    ...
    if HISTORY_PATH.exists():
        try:
            processed_dataset = pd.read_csv(HISTORY_PATH)
            print(
                f"[INFO] history.csv loaded: "
                f"{processed_dataset.shape[0]} rows, {processed_dataset.shape[1]} columns"
            )
        except Exception as e:
            print(f"[WARNING] Impossible de charger history.csv: {e}")
            processed_dataset = pd.DataFrame()
...
async def find_and_retrain_model():
    ...
    try:
        ...
        print(
            f"[RETRAIN] model updated in memory | "
            f"pop={selected_pop_name} | "
            f"algo={hyperparameters.get('forecasting_model')}"
        )
    finally:
        is_task_running = False
...
async def root():
    ...
    return {
        ...
        "history_file": str(HISTORY_PATH),
        "history_rows": history_rows,
        "selected_pop_name": selected_pop_name,
        "selected_horizon": selected_horizon,
        "selected_columns": selected_columns,
        "current_algo": None if hyperparameters is None else hyperparameters.get("forecasting_model")
    }
...
async def main_function(
    ...
    try:
        ...
        if len(target_columns) > 1:
            return JSONResponse(
                content={"error": "Select only one column please."},
                status_code=400
            )
        ...
        algo_name = hyperparameters.get("forecasting_model")
        saved_model_path = hyperparameters.get("saved_model_path")
        metadata_path = hyperparameters.get("metadata_path")
        ...
        return JSONResponse(
            content={
                ...
                "algo_name": algo_name,
                "hyperparameters": hyperparameters,
                "Best Params": hyperparameters,  # compatibilité avec anciens clients
                "training_elapsed_time": training_elapsed_time,
...
async def available_models(pop_name: str):
    try:
        ...
        available_list = [
            {"algo_name": algo_name, "model_path": model_path}
            for algo_name, model_path in available.items()
        ]
    ...
    if forecasting_model in ["RNN", "LSTM", "GRU", "ESN"]:
        ...
        if len(in_data) < required_history:
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Not enough historical data for {forecasting_model}. "
                    f"Need {required_history} values, got {len(in_data)}."
                )
            )
    else:
        if "window_length" not in hyperparameters:
