...
def load_saved_model(model_name, pop_name):
    ensure_model_dir()
    ...
    if model_name != "ARIMA" and not os.path.exists(metadata_path):
...
def list_available_models_for_pop(pop_name):
    ...
    for model_name in ["RNN", "LSTM", "GRU", "ESN", "ARIMA"]:
        model_path = get_model_filename(model_name, pop_name)
    ...
    if horizon == 1 and len(selected_columns) == 1:
        ...
        def objective(trial):
            ...
            if forecasting_model in ["LSTM", "GRU", "RNN"]:
                ...
                return np.sqrt(mean_squared_error(validY, validPredict))
        ...
        def objective(trial):
            forecasting_model = trial.suggest_categorical('forecasting_model', ['GRU'])
            ...
            model.fit(trainX, trainY)
            testPredict = np.array(esn_make_predictions(model, testX, horizon))
            _, new_testY = create_multistep_dataset(test, look_back, horizon)

            return np.sqrt(mean_squared_error(new_testY, testPredict[:len(new_testY), :]))
