...
def load_history_file():
    """
    Charge history.csv au démarrage de l'API.
...
async def main_function(
    ...
    """
    Entraîne automatiquement le meilleur modèle, le sauvegarde,
    puis le garde en mémoire pour /predict
    """
    global processed_dataset
    global selected_columns
    global selected_horizon
...
async def load_model_endpoint(
    ...
    """
    Charge un modèle déjà sauvegardé, sans réentraînement.
    L'historique est déjà chargé depuis history.csv au démarrage.
    """
    global selected_columns
    global selected_horizon
    global selected_pop_name
...
async def predict_endpoint(input_data: float):
    """
    Fait une prédiction avec le modèle déjà en mémoire.
    Utilise l'historique déjà chargé depuis history.csv.
    """
    global processed_dataset
    global selected_columns
    global selected_horizon
    ...
    if len(predictions) >= selected_horizon + 1 and len(in_data.tail(selected_horizon)) == len(predictions[prediction_index]):
